...
const PIN_LENGTH = 4;
/** Demo: "0000" is treated as wrong PIN; anything else succeeds */
...
export function APR03EnterPinPage() {
  ...
  const handleChange = (value: string) => {
    // Only allow digits, clamp to PIN_LENGTH
    ...
    <div className="min-h-screen bg-brand-neutral-100 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto pt-[72px] pb-[80px] flex items-center justify-center">
        <div className="px-5 flex flex-col items-center">
          ...
          {/* Subtitle */}
          <p
            className="text-[13px] text-brand-neutral-500 mb-8 text-center max-w-[260px]"
          ...
          <div className="w-full max-w-[240px] mb-4">
            ...
            {/* Visual dot field (tap to focus real input) */}
            <button
              type="button"
              onClick={focusInput}
              className={`w-full flex items-center justify-center gap-4 h-14 rounded-[6px] border bg-brand-neutral-0 transition-colors ${
                error
                  ? "border-brand-error-500"
                  : "border-brand-neutral-200 focus-within:border-brand-neutral-900"
              }`}
            >
              {Array.from({ length: PIN_LENGTH }).map((_, i) => (
                <div
                  ...
                  className={`w-3 h-3 rounded-full transition-all ${
                    i < pin.length
                      ? "bg-brand-neutral-900 scale-110"
          ...
          {showResetBanner && (
            <div className="w-full max-w-[240px] mb-4 flex items-center gap-2 px-3 py-2.5 rounded-[6px] bg-brand-success-50">
              <CheckCircle2
                size={14}
                className="text-brand-success-500 shrink-0"
              />
              <p
                className="text-[12px] text-brand-success-500"
                style={{ fontWeight: 400 }}
              >
                PIN updated. Please confirm approval.
              </p>
            </div>
          )}

          {/* ────────────────────────────────────
              Forgot PIN? link → APR-05
          ──────────────────────────────────── */}
          <button
      ...
      <div className="fixed bottom-0 left-0 right-0 z-10 bg-brand-neutral-0 border-t border-brand-neutral-200 px-5 pt-3 pb-4">
        <button
          ...
          disabled={submitting}
          className={`w-full h-11 rounded-xl text-[13px] flex items-center justify-center border-[1.5px] transition-colors ${
            submitting
