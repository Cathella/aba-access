...
const reasons = ["Not my dependent", "Wrong facility", "I don't consent"] as const;
...
export function APR06DeclineReasonPage() {
  ...
  const handleDecline = async () => {
    ...
    await supabase
      ...
      .update({ status: "Declined", decline_reason: selected ?? null, responded_at: new Date().toISOString() })
      .eq("id", requestId);

    const params = new URLSearchParams({ id: requestId, facility: facilityName });
    ...
    <div className="min-h-screen bg-brand-neutral-100 flex flex-col">
      <div className="fixed top-0 left-0 right-0 z-10 bg-brand-neutral-100 px-5 pt-6 pb-3 border-b border-brand-neutral-200">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-[17px] text-brand-neutral-900" style={{ fontWeight: 600 }}>Decline request</h2>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pt-[72px] pb-[80px]">
        <div className="px-5 pt-8 flex flex-col items-center">
          ...
          <p className="text-[13px] text-brand-neutral-500 text-center max-w-[280px] mb-8" style={{ fontWeight: 400 }}>
          ...
          <div className="w-full bg-brand-neutral-0 border border-brand-neutral-200 rounded-2xl px-4 py-3 mb-6">
            <div className="flex items-center justify-between">
              <span className="text-[12px] text-brand-neutral-500" style={{ fontWeight: 400 }}>Request from</span>
              <span className="text-[12px] text-brand-neutral-900" style={{ fontWeight: 500 }}>{facilityName}</span>
            </div>
          </div>

          <div className="w-full mb-8">
            <p className="text-[12px] text-brand-neutral-500 mb-3" style={{ fontWeight: 500 }}>
              Reason <span className="text-brand-neutral-400" style={{ fontWeight: 400 }}>(optional)</span>
            </p>
            <div className="flex flex-wrap gap-2">
              {reasons.map((reason) => (
                <button
                  key={reason}
                  onClick={() => setSelected((prev) => prev === reason ? null : reason)}
                  className={`px-3.5 py-2 rounded-full text-[12px] border transition-colors ${selected === reason ? "bg-brand-neutral-900 text-brand-neutral-0 border-brand-neutral-900" : "bg-brand-neutral-0 text-brand-neutral-700 border-brand-neutral-200"}`}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-10 bg-brand-neutral-0 border-t border-brand-neutral-200 px-5 pt-3 pb-4 flex gap-2.5">
        ...
        <button
          ...
          disabled={declining}
          className="flex-1 h-11 bg-brand-error-50 hover:bg-brand-error-500/20 text-brand-error-500 border-[1.5px] border-brand-error-500 rounded-xl text-[13px] flex items-center justify-center transition-colors disabled:opacity-60"
          style={{ fontWeight: 500 }}
        >
