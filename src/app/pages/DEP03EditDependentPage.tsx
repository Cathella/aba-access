...
/* ══════════════════════════════════════════════
   Constants & sample prefill
   ══════════════════════════════════════════════ */

const RELATIONSHIP_OPTIONS = ["Child", "Spouse", "Parent", "Other"];
const GENDER_OPTIONS = ["Male", "Female", "Other", "Prefer not to say"];

/* Prefilled with the first sample dependent (Ben) */
const PREFILL = {
  name: "Ben",
  relationship: "Child",
  dob: "2020-03-15",
...
export function DEP03EditDependentPage() {
  ...
  const [detailsOpen, setDetailsOpen] = useState(
    !!(PREFILL.allergies || PREFILL.conditions)
  );
  ...
  const handleSave = () => {
    setTouched(true);
    if (!isValid) return;
    /* Navigate back to whichever page opened the edit (DEP-04 or DEP-01) */
    navigate(-1);
  };
  ...
  return (
    <div className="min-h-screen bg-brand-neutral-100 flex flex-col">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-10 bg-brand-neutral-0 border-t border-brand-neutral-200 px-5 pt-3 pb-5 flex gap-3">
        {/* Secondary (outline) – Remove dependent */}
        <button
          onClick={() => navigate("/dep-06")}
        ...
        <button
          onClick={handleSave}
          className="flex-1 h-11 rounded-xl text-[14px] flex items-center justify-center border-[1.5px] bg-brand-primary-300 hover:bg-brand-primary-400 text-brand-neutral-900 border-brand-neutral-900 transition-colors"
          style={{ fontWeight: 500 }}
        >
          Save changes
        </button>
